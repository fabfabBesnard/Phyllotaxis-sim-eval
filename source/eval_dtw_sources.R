...
#"N.align"          ->    Number of intervals (in the REF sequence) that are aligned to the current (TEST) interval. E.g: a split aligns to several ref intervals (N.align>1); NA for Tails and some other cases
#"align.score"      ->    Score of the above interval number prediction, range [0-1]. 1 means that number prediction is correct, error implies a score lower than 1
...
##################################
#intern sub function
get_consecutive_segments=function(myvector, output=c("all", "max")){
  #DESCRIPTION: compute the length of the segments made by consecutive values of vector in the way they are ordered
  
  output=match.arg(output)
  
  vector.shift=myvector[-length(myvector)]+1
  breaks=c(which(myvector[-1]!=vector.shift), length(myvector))
  segments.length=c()
  pointer=0
  for (b in breaks){
    l=b-pointer
    segments.length=c(segments.length,l)
    pointer=b
  }
  
  if (output=="max"){return(max(segments.length))}
...
#The quality of the predicted alignment can be evaluated using the following 6 information and data:
        ...
        # N.dtw.align: the number of intervals aligned by the dtw (excluding "free-end" Tails)
        ...
        # longest.correct.segment: provide the length of the longest continuous correct segment
...
summarize_prediction_eval=function(comparison.df, true_align.df, verbose=TRUE){
  ...
  #[output]: a 8-column dataframe containing info and metrics (7 rows), one row per PlantID, the first column is the PlantID
  ...
  for (plantid in levels(comparison.df$PlantID)){
    ...
    #Length of dtw alignment (drop free-end "Tails" that have not been aligned to reference sequence)
    N.dtw.align=N.test-sum(comparison_plant$dtw.predict=="T")
    ...
    NonMatch.correct.ratio=sum(comparison_plant[comparison_plant$dtw.eval=="correct" & !(comparison_plant$dtw.true %in% c("~","=")),]$dtw.eval=="correct")/
      nrow(comparison_plant[!(comparison_plant$dtw.true %in% c("~","=")),])
    #Length of the longest continuous correct segment
    correct.idx=which(comparison_plant$dtw.eval=="correct")
    longest.correct.segment=get_consecutive_segments(correct.idx, output = "max")
    
    plant_summary=cbind.data.frame(plantid, 
                                   N.ref, N.test, 
                                   N.dtw.align, Correct.ratio, 
                                   Match.correct.ratio, NonMatch.correct.ratio,
                                   longest.correct.segment)
...
# 16. LONGEST.MATCH [numeric]: length of the longest continuous matching segment (without any operations)
# 17. ANGLES.difference [numeric]: mean difference between angle values in reference and test sequence among intervals that match
# 18. ANGLES.sd [numeric]: standard deviation between angle values in reference and test sequence among intervals that match
# 19. INTERNODES.difference [numeric]: mean difference between internode values in reference and test sequence among intervals that match
# 20. INTERNODES.sd [numeric]: standard deviation between angle values in reference and test sequence among intervals that match
...
summarize_align_prediction=function(dtw.res, seq.ref, seq.test, count.as=c("number","ratio"), verbose=FALSE){
  ...
  for (plantid in levels(dtw.res$PlantID)){
    ...
    if (nrow(plant.dtw.res[plant.dtw.res$dtw %in% c("~","="),])>0){
      ...
      matching.idx=plant.dtw.res[plant.dtw.res$dtw %in% c("~","="),]$test
      longest.match=get_consecutive_segments(matching.idx, output = "max")
    }
    #Splits
