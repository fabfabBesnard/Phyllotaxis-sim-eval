...
## Convert dtw results  ##
##########################
convert_dtw_results=function(my_res, seq.ref, seq.test){
  ...
  out=cbind(my_res[,2], my_res[,1]+1, my_res[,c(4,3)]+1, my_res[,c(5,6)] )
  ...
  for (id in levels(PlantID)){
    ...
    if (max(out_subset$reference, na.rm = TRUE) < Nref){
      ...
      out_subset=duplicate_line(out_subset, nrow(out_subset), (Nref - last.ref.idx) )
      #update ref index(es) in the added rows
      #print(last.ref.idx)
      #print(out_subset[which(out_subset$reference >= last.ref.idx),])
      #print(seq((last.ref.idx),Nref))
      out_subset[which(out_subset$intervals == max(out_subset$intervals)),]$reference=seq((last.ref.idx),Nref)
      #update test index(es): they are NAs
...
summarize_prediction_eval=function(comparison.df, true_align.df, verbose=TRUE){
  ...
  #true_align.df [input]: dataframe with [$PlantID	$reference	$modified	$dtw], typical align_interval.csv generated by Rscript test_dtw.R
